...
public interface CGMSessionRunTimeCallback {
	 ...
	 * If CRC check has failed, the {@link #onContinuousGlucoseMonitorSessionRunTimeReceivedWithCrcError(BluetoothDevice, Data)}
	 ...
	 */
	void onContinuousGlucoseMonitorSessionRunTimeReceived(@NonNull final BluetoothDevice device, final int sessionRunTime, final boolean secured);

	/**
	 * Callback called when a CGM Session Run Time packet with E2E field was received but the CRC check has failed.
	 *
	 * @param device target device.
	 * @param data   CGM Session Run Time  packet data that was received, including the CRC field.
	 */
	default void onContinuousGlucoseMonitorSessionRunTimeReceivedWithCrcError(@NonNull final BluetoothDevice device, final @NonNull Data data) {
		// ignore
	}
}
